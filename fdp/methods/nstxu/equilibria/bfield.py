...
def bfield(obj, radius=None, z=None, time=None):
    ...
    if not obj.isContainer():
        warnings.warn('Must call bfield() from equilibrium container, returning')
        return
    ...
    # defaults
    if not radius: radius = 1.4   # major radius in m
    if not z: z = 0.0   # height in m
    if not time: time = 0.5   # time in s
